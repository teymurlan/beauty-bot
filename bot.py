...
def main_menu_kb(user_id: int) -> ReplyKeyboardMarkup:
    # ❌ Отменить запись УБРАНА из главного меню и перенесена в профиль
    kb = [
        ["💅 Записаться", "💳 Цены"],
        ["👩‍🎨 Обо мне", "📍 Контакты"],
...
async def about_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if MASTER_PHOTO:
        ...
        remember_bot_msg(context, msg.message_id)
        await flow_show(update, context, "Выберите действие 👇", kb_about_inline())
    else:
        await send_clean(update, context, about_master_text(), reply_markup=about_reply_kb())
        await flow_show(update, context, "Выберите действие 👇", kb_about_inline())
...
def _normalize(txt: str) -> str:
    t = (txt or "").strip()
    low = t.lower()
    if low in ("/admin", "admin", "админ"):
        return "🛠 Админ-панель"
    if low in ("/book", "book", "записаться"):
        return "💅 Записаться"
    if low in ("/menu", "menu", "меню"):
        return "🏠 Меню"
    if low in ("/prices", "prices", "цены", "прайс"):
        return "💳 Цены"
    if low in ("/profile", "profile", "профиль"):
        return "👤 Профиль"
    if low in ("/faq", "faq", "вопросы"):
        return "❓ Вопросы и ответы"
    if low in ("/about", "about", "обо мне"):
        return "👩‍🎨 Обо мне"
    if low in ("галерея", "мои работы"):
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if stage == STAGE_REG_NAME:
        if _looks_like_button(txt):
            await send_clean(update, context, "✍️ Во время регистрации нужно <b>ввести имя текстом</b>.", remove_reply=True)
            await delete_user_input(update, context)
            return
        ...
        await send_clean(update, context, "📱 Отправьте номер кнопкой ниже (или напишите +79991234567).", reply_markup=phone_request_kb())
        await delete_user_input(update, context)
        return
    ...
    if stage == STAGE_REG_PHONE:
        if _looks_like_button(txt):
            await send_clean(update, context, "📱 Нужен номер телефона. Нажмите кнопку «📱 Отправить номер» или напишите номер.", reply_markup=phone_request_kb())
            await delete_user_input(update, context)
            return
        phone = parse_phone(raw)
        if len(re.sub(r"\D", "", phone)) < 10:
            await send_clean(update, context, "Нажмите «📱 Отправить номер» или напишите номер в формате <code>+79991234567</code>.", reply_markup=phone_request_kb())
            await delete_user_input(update, context)
            return
        name = context.user_data.get("reg_name", update.effective_user.full_name or "Клиент")
        store.upsert_user(uid, update.effective_user.username or "", name, phone)
    ...
    if stage == STAGE_ASK_MASTER:
        ...
        try:
            ...
            await send_clean(update, context, "⚠️ Не удалось отправить мастеру. Проверьте ADMIN_ID и что мастер нажал /start.", reply_markup=main_menu_kb(uid))
        context.user_data["stage"] = STAGE_NONE
        await delete_user_input(update, context)
        return
    ...
    if txt == "📩 Вопрос мастеру":
        ...
        await send_clean(update, context, "✍️ Напишите ваш вопрос одним сообщением.\n\nМожно отказаться кнопкой ниже или /cancel", reply_markup=kb_cancel_question_inline())
        await delete_user_input(update, context)
        return

    # cancel in profile (moved here)
    if txt == "❌ Отменить запись":
        ...
        if not items:
            await send_clean(update, context, "У вас нет активных записей 🙂", reply_markup=profile_kb(uid))
            await delete_user_input(update, context)
            return
        did = set_draft(context)
        rows = []
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("UCANCEL:"):
        ...
        if not must_draft(context, did):
            await flow_show(update, context, "Кнопки устарели. Откройте отмену в профиле заново 🙂")
            return
        ...
        await send_clean(update, context, "👤 Профиль 👇", reply_markup=profile_kb(uid))
        return
