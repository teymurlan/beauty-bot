...
def main_menu_kb(user_id: int) -> ReplyKeyboardMarkup:
    # Убрали кнопку отмены записи отсюда
...
def kb_days(did: str) -> InlineKeyboardMarkup:
    ...
    d = now_local().date()
    for i in range(31):
        iso = (d + timedelta(days=i)).isoformat()
...
def build_confirm_text(context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    if not title or not d_iso or not t: return "⚠️ Ошибка сценария."
...
def reschedule_all_reminders(app: Application):
    ...
    for b in store.list_for_reminders():
        dt = datetime.combine(date.fromisoformat(b.book_date), dtime(*map(int, b.book_time.split(":")))).replace(tzinfo=tz)
        if dt > now: schedule_reminder(app, b.id, b.book_date, b.book_time)
...
def _normalize_btn(text: str) -> str:
    t = (text or "").strip()
    # Очищаем текст от эмодзи и спецсимволов для надежного сравнения
...
async def profile_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not u:
        ...
        return
    # Кнопка отмены записи теперь здесь
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if stage == STAGE_REG_PHONE:
        ...
        context.user_data["stage"] = STAGE_NONE
        await send_clean(update, context, "Регистрация завершена!", reply_markup=main_menu_kb(uid))
        await delete_user_input(update, context); return
    if stage == STAGE_MANUAL_TIME:
        ...
        await flow_show(update, context, "Добавьте комментарий:", kb_comment(get_draft(context)))
        await delete_user_input(update, context); return
    ...
    if stage == STAGE_ASK_MASTER:
        ...
        await context.bot.send_message(ADMIN_ID, f"📩 <b>Вопрос мастеру</b>\nОт: {h(who)}\n\n{h(txt_raw)}", parse_mode="HTML")
        await send_clean(update, context, "Ваш вопрос отправлен!", reply_markup=main_menu_kb(uid))
        context.user_data["stage"] = STAGE_NONE
        await delete_user_input(update, context); return

    if txt == "💅 Записаться":
        if not u: await flow_show(update, context, "Сначала зарегистрируйтесь.", kb_start_for_new_user())
        else: await flow_show(update, context, "Выберите категорию:", kb_service_cats(set_draft(context)))
        ...
        await send_clean(update, context, "Напишите ваш вопрос:", reply_markup=ReplyKeyboardRemove())
    elif txt == "🏠 Меню": await start(update, context)
    elif txt == "🛠 Админ-панель" and is_admin(uid): await flow_show(update, context, "Панель администратора:", kb_admin_panel())
    else: await send_clean(update, context, "Выберите действие в меню 👇", reply_markup=main_menu_kb(uid))
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "REG_START": context.user_data["stage"] = STAGE_REG_NAME; await flow_show(update, context, "Как к вам обращаться?"); return
    if data == "BOOK_START": await flow_show(update, context, "Выберите категорию:", kb_service_cats(set_draft(context))); return
    if data == "CANCEL_LIST":
        items = store.list_user_upcoming(uid)
        if not items: await flow_show(update, context, "У вас нет активных записей."); return
        ...
        await flow_show(update, context, "Выберите запись для отмены:", InlineKeyboardMarkup(rows)); return
    if data.startswith("CAT:"):
        did, cat = data.split(":")[1:]
        if must_draft(context, did): context.user_data["book_cat"] = cat; await flow_show(update, context, "Выберите услугу:", kb_services(did, cat))
    if data.startswith("SVC:"):
        ...
        if must_draft(context, did):
            ...
            await flow_show(update, context, f"Услуга: {t}\nВыберите дату:", kb_days(did))
    if data.startswith("DAY:"):
        did, iso = data.split(":")[1:]
        if must_draft(context, did): context.user_data["book_date"] = iso; await flow_show(update, context, f"Дата: {iso}\nВыберите время:", kb_times(did, iso))
    if data.startswith("TIME:"):
        did, t = data.split(":")[1:]
        if must_draft(context, did):
            ok, res = is_time_allowed_for_booking(context.user_data.get("book_date"), t)
            if ok: context.user_data["book_time"] = res; context.user_data["stage"] = STAGE_BOOK_COMMENT; await flow_show(update, context, "Добавьте комментарий:", kb_comment(did))
    if data.startswith("CONFIRM:"):
        ...
        if must_draft(context, did):
            ...
            if done:
                ...
                await flow_show(update, context, "Запись создана! Мастер уведомлен."); await context.bot.send_message(ADMIN_ID, f"Новая запись #{bid} от {u.full_name}", reply_markup=kb_adm_confirm_cancel(bid, user_link(uid, q.from_user.username), uid))
                clear_draft(context)
    if data.startswith("UCANCEL:"):
        did, bid = data.split(":")[1:]
        if must_draft(context, did): store.set_booking_status(int(bid), "cancelled"); await flow_show(update, context, "Запись отменена."); clear_draft(context)
...
def main():
    ...
    app.add_handler(MessageHandler(filters.CONTACT, lambda u, c: handle_text(u, c)), group=1) # Handle contact as text
